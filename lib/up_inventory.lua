...
local function inspect_instrument(inst, ii)
...
function up_inventory.scan(song, yield_fn, on_progress, on_found)
  ...
  for ii = 1, #song.instruments do
    ...
    local rec = inspect_instrument(song.instruments[ii], ii)
    if rec then
      table.insert(entries, rec)
      if on_found then
        on_found(rec)
...
function up_inventory.scan_instrument_device(inst)
  ...
  return inspect_instrument(inst, ii)
end
